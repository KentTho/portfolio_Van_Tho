...
- **V1 (CV-driven, Prompt 12R/V1-finalize):** ✅ **6-block recruiter-first landing** (`#home #about #projects #career #skills #contact`; Focus/Principles/Articles removed from landing — article domain/detail routes retained). **Real CV content ingested into Neon Development via Admin UI** (owner_admin authenticated): profile, Education (NTTU/Software Engineering/2022–nay), 15 skills (6 groups), 6 technologies, published **Expense Tracker** project. Two integration mapping fixes (`getTechGroups`←skills, `education`←profile) so ingested data reaches `#skills`/`#career`. Populated visual QA PASS. Premium motion (cursor halo, kinetic type, magnetic CTA, pointer-tilt portrait, intro). CI green; PR #6 ready to merge (Owner approved Production). Pending Owner: Experience details, public-safe resume. No migration (25 tables/ledger 6).
