...
- **Active branch:** `integration/pre-fe-foundation` @ `60c8d90` (tạo từ verified Wave-05 HEAD `e7f8e02`; merge Wave-04 public `94f547e`). `feat/wave-05-cms-foundation` @ `e7f8e02` = origin, in sync. Production locked; no prod deploy/migration/DNS.
...
- **E2E (Playwright 1.56.1):** ✅ **Public 9/9 VERIFIED** headless trên app thật + live Neon (load vi/en, no admin-nav/draft/archive leak, anon `/admin`→`/admin-login`, locale redirect). Authenticated specs (admin allow, project/article lifecycle) chờ storageState OAuth headed = **PENDING_OPERATOR**. Validation: **check:env 18/18 · 184 unit +6 skip · 10 arch · secret-free build xanh** · live public read smoke (2) trên Neon Dev.
