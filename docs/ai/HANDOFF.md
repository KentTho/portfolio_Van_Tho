...
- **⚠️ `STOP_PRODUCTION_DATA_ENVIRONMENT_DECISION_REQUIRED`** — `PRODUCTION_DATABASE_TARGET = SAME_AS_DEVELOPMENT` (secret-safe behavioral proof: Production renders the exact rows only ever ingested into Neon Development; Vercel has one `DATABASE_URL`/`_UNPOOLED` binding shared `Production, Preview`, no separate Development binding). Owner must decide: dedicated prod Neon branch vs shared-for-V1. AI performed NO migrate/copy/switch; no secret/URL printed.
