...
- **CODE_HEAD / Remote HEAD:** `0c89f53` (docs commit for this checkpoint updates it — see git log; LOCAL == REMOTE after push).
- **Current objective:** Prompt 12R final CV-driven build — Neon Dev content ingestion → Waves E–H → populated QA → Owner Visual Acceptance.
- **Current gate:** `PENDING_OPERATOR_ADMIN_LOGIN` — Owner-chosen ingestion path = **Admin UI (authenticated)**; Owner authorized Claude to drive the Admin forms AFTER an authenticated session exists. **Blocking operator step:** `pnpm e2e:auth-setup` (headed GitHub OAuth once → `tests/.auth/owner.json` storageState). Then Claude drives Admin (Playwright + storageState) → Server Action → use-case → repo → Neon **Development** (verified: APP_ENV=development, both Neon URLs, NOT production).
- **Latest validation (Wave D IA rebase):** typecheck · lint · secret-free build · public E2E **7/7** (new 6-block anchors). 25 tables / ledger=6 (no migration).
- **Owner decisions (locked):** ingestion via Admin UI (auto-driven post-login, no raw SQL/backdoor/migration, dev-only); **domain gaps → temporary UI/config fallback for this RC (no migration)**; Expense Tracker only; Experience pending Owner details; phone PRIVATE; primary resume = Fullstack PDF but **NOT served publicly** (contains phone) → Resume CTA absent.
- **CONTENT_STORAGE plan (ingest via Admin):** profile (full_name/professional_title/location/public_email) · Education (NTTU, Software Engineering, 10/2022–present, GPA 3.09) · Skills+Technologies (normalized) · Expense Tracker project (merged from both CVs, all facts Owner-verified real). **NOT ingested:** summary/headline (no field), GitHub/LinkedIn (no social_links domain), Awards (`certifications` ≠ Award) → **domain gaps** below; Experience (pending).
- **DOMAIN GAPS (proven, no migration in 12R):** `SUMMARY_DOMAIN_GAP` · `SOCIAL_LINKS_DOMAIN_GAP` · `AWARDS_DOMAIN_GAP`. RC strategy = UI/SITE-config fallback (About summary → dict/SITE; contact → SITE GitHub; awards omitted). **FUTURE ARCHITECTURE ITEM (Owner-mandated):** `PROFILE_AND_PUBLIC_IDENTITY_DOMAIN_EXPANSION` — add profile summary/headline + social-links + (if CMS-managed) awards via a dedicated migration wave (ports/use-cases/repos/Admin CRUD/public read model/tests/arch validation). Before Production: decide CMS-managed authority vs SITE/config authority. Temporary fallback must NOT persist indefinitely without that decision.
- **Reference audit done:** `docs/image_demo_portfolio/{portfolio,About,Experience-Education(=projects cards),hero-section,Education}.png` — adapt vertical rhythm, portrait prominence, split-About, card-structured project, timeline career; reject purple (→blue/gold), fake testimonials/screenshots, false numbering, card/eyebrow spam.
- **NEXT EXACT STEP:** Owner runs `pnpm e2e:auth-setup` (interactive OAuth). Then: (1) Claude ingests profile/Education/Skills/Expense-Tracker via Admin (Playwright+storageState), verify each write masked. (2) Wave E (Expense Tracker featured + Career/Education). (3) Wave F (Skills/Credentials + Contact/Footer). (4) Wave G (cross-section choreography). (5) Wave H (populated QA + docs). (6) Owner Visual Acceptance.
