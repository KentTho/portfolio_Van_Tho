...
| **Vivid substrate** (global visual authority) | `src/components/public/cosmic-background.tsx` (client) | **V2 section-aware substrate** (adapted from `vivid_co`, NOT cloned — Owner Q01A–Q08A): graphite-veil depth (blue-tinted slate, never flat black) + **brand-recoloured chromatic PRISM** (CSS only — a **crisp optical field**: refractive glass fragments + a chromatic light streak, brand blue + restrained gold; **no red/lime/purple/rainbow**, **no WebGL**) + **scene state per section** (home=identity spotlight · about=convergence · projects=technical grid-plane · career=vertical trace · skills=capability field · contact=focus collapse · footer=system rest) so the *environment* visibly changes between chapters (fixes the homogeneous-substrate root cause). ONE centre-band IntersectionObserver (6 anchors) + one footer observer; prism repositions/rescales per scene (shared language, different role §11). **Pointer-reactive light** (CSS var `--vx/--vy`, one rAF listener, `pointer:fine`+motion only). One slow shimmer (~7s). Transitions use `--ease-scene-focus`. Decorative (aria-hidden, pointer-events:none) — never aberration under text. Reduced-motion → static (global gate); mobile → simplified blur + no pointer/shimmer (CSS `@media`). Fixed `-z-10`. |
...
- **`Reveal` primitive (`reveal.tsx`):** configurable `direction (up|left|right)` · `distance (30)` · `duration (0.72)` · `amount (0.22)` · optional `blur`. **Replayable with hysteresis** (`use-replayable-reveal.ts`): content is in the DOM from first render; it enters on the inner focus band and re-arms only after leaving an **expanded exit band** (enter threshold ≠ exit threshold) → **fast-scroll safe** (never blank, never hide-while-reading, no boundary flicker), replays on genuine re-entry, no Hero IntroCurtain replay. **Product state (Career tab, Contact copy) is never remounted** on replay. Trigger `amount:0.22` = fires when the element is actually visible.
