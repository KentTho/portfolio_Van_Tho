...
| **PUBLIC PRESENTATION** | **74** | **SINGLE LANDING per locale** (`/vi` `/en`): anchored sections `#home…#contact` + locale-aware scroll-spy nav; new About / Experience & Education / Latest Articles sections compose former `/about /resume /articles` from the SAME live Neon read model (`NeonPortfolioRepository`, single runtime authority, no fixture fallback, honest empty states). Consolidated routes → `/#anchor`; `/projects/[slug]` + `/articles/[slug]` preserved. `#contact` = real email/social (form → Wave 06A). **Public browser E2E 7/7** (single-landing sections + redirects, no admin/draft/archive leak). typecheck/lint/184+6/arch 10/10/secret-free build green | real published content pending (Owner authors via Admin — Neon Dev ~0 rows ⇒ truthful empty states); **brand visual system = Prompt 12** (logo palette, `vantho.png` hero, motion, a11y); Preview smoke pending | Prompt 12 brand visual system → authed E2E + Vercel Preview |
