...
| **PUBLIC PRESENTATION** | **72** | Wave 04 i18n vi/en shell (Home/projects/articles/about/resume/contact, SEO, motion) **integrated** + **wired to live Neon** (`NeonPortfolioRepository`, single runtime authority, no fixture fallback; on-demand render keeps build secret-free). **Public browser E2E 9/9** (no admin/draft/archive leak). 5 mapper unit tests + live read smoke | real published content pending (Owner authors via Admin — Neon Dev ~0 rows ⇒ truthful empty states); tech-matrix/case-study richer fields render when content exists; Preview smoke pending; visual redesign deferred | Owner authors content → Vercel Preview → Wave 04 Phase 2 visual redesign |
