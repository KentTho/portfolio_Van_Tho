...
| **V2_PUBLIC_VISUAL_ENHANCEMENT** | 🚧 **IN PROGRESS — Hero+Menu+About** | branch `feat/v2-hero-menu-enhancement` (PR #9): `#home` Hero corrected (2-line name, portrait anchor, single CTA, darker canvas) + Menu verified (scroll-spy all 6 blocks, active≠hover) + `#about` redesigned (editorial split, backlit fact-panel). Hydration forensic = EXTERNAL_BROWSER_EXTENSION_MUTATION_CONFIRMED (clean env 0 errors). Validation GREEN, e2e:public 7/7. **Not merged** (Owner Visual Acceptance). | Projects/Career/Skills/Contact V2 passes pending; Owner acceptance of Hero/Menu/About | next section after acceptance |
