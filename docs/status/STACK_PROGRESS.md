...
| **V2_PUBLIC_VISUAL_ENHANCEMENT** | 🚧 **IN PROGRESS — Hero/Menu/About + Career MERGED; Contact/Footer in review** | **Hero+Menu+About + Career = MERGED to main `16878f5` + Production LIVE.** **Contact/Footer** (branch `feat/v2-contact-footer-enhancement`, PR open): conversion-first Contact (real mailto + robust copy-email state machine, 0 width-jump, aria-live) + Footer closure + Back-to-Top; hydration hardened (`EXTERNAL_BROWSER_EXTENSION_MUTATION_CONFIRMED`, regression guard). Validation GREEN, **test 198/6**, **e2e:public 9/9**. **Not merged** (Owner Visual Acceptance per section). | Projects/Skills V2 pending; `@remixicon/vue` (Owner remove); `PENDING_OWNER_EXPERIENCE_DETAILS` | Owner review Contact/Footer → next section |
