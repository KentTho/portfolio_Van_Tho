...
| **FRONTEND** | **45** | Wave 04 Phase 1 (local): i18n vi/en SSG shell, Home (hero + tech matrix + focus + featured + principles + CTA), projects/articles/about/resume/contact, cosmic design system, motion + reduced-motion, SEO (metadata/robots/sitemap/JSON-LD), typed content BE via ports; tests 46/46; build green. **No admin surface on public.** | Preview not yet smoke-verified; real brand SVGs (Devicon) pending; 3D deferred; projects/articles are labelled SAMPLE | Vercel Preview smoke → real content + Devicon logos → selective 3D |
