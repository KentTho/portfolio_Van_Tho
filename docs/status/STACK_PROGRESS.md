...
| **V2_PUBLIC_VISUAL_ENHANCEMENT** | 🚧 **IN PROGRESS — Hero+Menu+About (approved) + Career** | **Hero+Menu+About** (PR #9) = Owner-**APPROVED**; merge pending GitHub API outage. **`#career`** (branch `feat/v2-career-experience-education`): Education-first central-axis timeline + dormant two-tab [Experience\|Education] state machine (`listEducation()` port method, live Neon, no fabrication). Validation GREEN, **test 195/6**, e2e:public 7/7, BOTH-mode unit+harness verified (0 CLS). **Not merged** (Owner Visual Acceptance per section). | Projects/Skills/Contact V2 pending; `PENDING_OWNER_EXPERIENCE_DETAILS` (Experience rows) | merge PR #9 when API up → Owner review Career → next section |
