...
| **V2 Public visual enhancement** | per-section visual enhancement of the 6 blocks (Hero/About/Projects/Career/Skills/Contact) on live data | 🚧 **IN PROGRESS** — Hero+Menu+About **approved** (PR #9, merge pending GitHub API outage); **Career** done (branch `feat/v2-career-experience-education`, Education-first timeline + experience-ready two-tab, **not merged**). Projects/Skills/Contact pending. | UI authority `PUBLIC_LANDING_DESIGN_MAP.md`; refs `docs/image_demo_portfolio/*` + `HEROAUDIT`/`Menu_audit`/`ABOUT01audit`/`EXPERIENCE_EDUCATIONaudit` |
