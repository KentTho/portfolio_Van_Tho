...
| **V2 Public visual enhancement** | per-section visual enhancement of the 6 blocks + global nav/motion on live data | 🚧 **IN PROGRESS** — **6 blocks + Footer MERGED** to `main` `9939ec5` (Production live); **Global Navigation + Motion** done (branch `feat/v2-global-navigation-motion-system`, PR #13: compact/expanded icon-capsule nav + **visible-premium MASTER-MOTION-02** per-section choreography + `@remixicon/vue` removed; MERGED to main `68e6f9e`, Production live). Projects/Skills per-section V2 pending. **Vivid visual substrate** in review (branch `feat/v2-vivid-visual-substrate-and-motion-rebase-03`, PR open — section-aware substrate + brand chromatic prism, CSS/SVG no-WebGL, not merged). | UI authority `PUBLIC_LANDING_DESIGN_MAP.md`; refs `docs/image_demo_portfolio/*` + `HEROAUDIT`/`Menu_audit`/`ABOUT01audit`/`EXPERIENCE_EDUCATIONaudit`/`CONTACTFOOTER01audit`/`Menu-Update`/`MASTERSCROLL` |
